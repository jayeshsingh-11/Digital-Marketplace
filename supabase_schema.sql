...
-- 1. Tables Setup (Safe creation if missing)
...
create table if not exists public.order_products (
  id uuid default uuid_generate_v4() primary key, -- Added PK to avoid composite key issues with some tools
...
-- 4. Storage Buckets (Safe Insert)
insert into storage.buckets (id, name, public) values ('media', 'media', true) on conflict (id) do nothing;
insert into storage.buckets (id, name, public) values ('product_files', 'product_files', false) on conflict (id) do nothing;
...
create policy "Users can add items to their orders" on public.order_products for insert with check (
  exists (select 1 from public.orders where id = order_products.order_id and user_id = auth.uid())
);
...
create policy "Avatars Auth Update" on storage.objects for update using (bucket_id = 'avatars' and auth.uid() = owner);
create policy "Avatars Auth Delete" on storage.objects for delete using (bucket_id = 'avatars' and auth.uid() = owner);
